...
* (Optional) For automated testing create `.env.test.local` file and add `TEST_ID`, `TEST_PASS`, `API_URL`, `API_KEY` env variables.
* (Optional) Set `TEST_ID` and `TEST_PASS` to the credentials for a test user in your Alma system.  These credentials will be used while running automated tests (in `.env.test.local`).
